...
# ╔═╡ 03803d62-db8f-4110-a3d2-e44f69454c31
...
function var(D::Vector{Float64})
	...
	return sum( (D.-(sum(D)/length(D))) .^2) / (length(D)-1)
...
function stddev(D::Vector{Float64})
	...
	return sqrt( sum( (D.-(sum(D)/length(D))) .^2) / (length(D)-1) )
...
let # 4b
	...
	dataset = sample(gd,n)
	println("this is sample mean")
	println(mean(dataset))
...
# ╔═╡ e9ded205-eb91-4001-939a-e69cae810fb1
let 
	
V=[0.16764701845221358,0.41178851786062076,0.07116757291228794,-0.13184956339831044,0.4243379621936974]
	
	println("this is sample variance")
	var(V)
...
# ╔═╡ f74a4f31-4e69-4be9-88a4-b1b2e6396e0b
let
	v=[-0.030971354998008283,-0.12004653583081985,-0.007185097669964636,-0.06464283854843568,0.0731386456593175]
	var(v)
